...
int slice_integer(Slice s, long int *val) {
  *val = 0;
  ...
  if (items != 1 || (uint32_t)bytes != s.len)
    return 0;
  return 1;
}
...
Slice slice_find_slice(Slice s, Slice t) {
  ...
  const char *ptr = memmem(s.ptr, s.len, t.ptr, t.len);
  if (ptr) {
    unsigned head = ptr - s.ptr;
    return slice_from_memory(ptr, s.len - head);
  }
#else
...
static bool slice_tokenize(Slice src, Slice s, SliceLookup *lookup) {
  ...
  uint32_t k = j;
  for (; k < len; ++k) {
    if (lookup->map[(int)ptr[k]]) {
      break;
    }
  }

  // produce current result
  lookup->result = slice_from_memory(ptr + j, (k < len) ? (k - j) : (len - j));
